...
def create_short_url(payload: ShortenRequest, request: Request, db: Session = Depends(get_db)) -> ShortenResponse:
    # validate url format
    url = payload.url.strip()
    if not is_valid_http_url(url):
        raise HTTPException(status_code=400, detail="Invalid URL format (expected http(s)://...)")
